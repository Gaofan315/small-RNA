...
./mustard-master/MuStARD.pl predict --modelDirName predict_results --model Drosha_independent.CNNonRaw.hdf5 --modelType CNN --inputMode sequence,RNAfold --chromList all --targetIntervals $in\.sorted.bed --genome hg19.fa --classNum 2 --consDir NaN --staticPredFlag 1 --dir predict_$in\_out 2>log.$in
